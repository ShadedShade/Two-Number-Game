...
router.get('/latestDraw', (req, res) => {
    // request all games 
    db.query("Select   draw.DrawCombo, gametype.gamename From numbers.draw inner join numbers.gametype on draw.gameid = gametype.gameid  Where (date(now())>= draw.drawdate) group by draw.gameid order by draw.drawdate desc;", (err, result) => {
        if (err) {
            res.send({ err: err });
        }
        if (result.length > 0) {            
